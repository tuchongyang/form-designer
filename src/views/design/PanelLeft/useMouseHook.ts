...
function startAdd() {
  const defaultModelData: ModuleType | undefined = defaultData.find((a) => a.type == dragEvent.moduleType)
  if (!defaultModelData) {
    return message.info("模块未找到")
  }
  const data = Utils.deepCopy<ModuleType>(defaultModelData)
...
function toAdd(module: ModuleType) {
  // const store = useStore()
  // const store = useStore<GlobalDataProps>()
  store.commit("moduleAdd", { module, index: dragEvent.moduleIndex })
  dragEvent.moduleIndex = -1
}
